// NOTES – 2025-10-01
// Implemented recursive folder scanning (scanFolder)
// Added MD5 hashing function (computeMD5FromFile)
// MD5 printed to console for each file
// Signature verification not working yet (WinVerifyTrust issues)
// Next step:  implement code to secure .dat file
// Next step: add SHA-1 / SHA-256 support
// Next step: write results into BaseScan.dat

#include <iostream>     // for console input/output (cout, cerr, wcout)
#include <windows.h>    // Windows API (file system, console, etc.)
#include <fstream>      // for file streams (ofstream to write output)
#include <iomanip>      // for formatting (setw, setfill, hex)
#include <string>       // for using std::string and std::wstring
#include <wincrypt.h>   // Windows cryptography API (hashing)
#include <sstream>      // for string streams (ostringstream)
#include <iomanip>      // (duplicate include, safe but unnecessary)
#include <vector>       // for std::vector (buffer when reading files)
#include <Softpub.h>    // (not used now, was for WinVerifyTrust)
#include <tchar.h>      // (not really used in your current code)

// #include "Hash_Utils.h"   // optional header if you separate code into modules
...
// ------------------------------
// Function: computeMD5FromFile
// Reads a file and calculates its MD5 hash
// ------------------------------
